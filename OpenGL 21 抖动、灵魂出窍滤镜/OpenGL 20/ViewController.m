...
- (HJFilterBar *)toolBar{
    if (!_toolBar) {
        _toolBar = [[HJFilterBar alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - 100, self.view.frame.size.width, 90) items:@[@"无",@"灰度滤镜",@"颠倒滤镜",@"旋涡滤镜",@"马赛克",@"六边形马赛克",@"三角形马赛克",@"灵魂出窍",@"闪白",@"抖动",@"毛刺"]];
...
-(void)setSoulShader{
    ...
    [self setupShaderProgramWithShaderName:@"soul"];
}

/// 闪白
-(void)setShineWhiteShader{
    
    [self setupShaderProgramWithShaderName:@"shineWhite"];
}

/// 抖动
-(void)setShakeShader{
    
    [self setupShaderProgramWithShaderName:@"shake"];
}

/// 撕裂
-(void)setGlitchShader{
    
    [self setupShaderProgramWithShaderName:@"glitch"];
}
...
- (void)toolBarSelected:(NSInteger)index{
    ...
    switch (index) {
        ...
        case 7:
            ...
            break;
        case 8:
            [self setShineWhiteShader];
            break;
        case 9:
            [self setShakeShader];
            break;
        case 10:
            [self setGlitchShader];
            break;
        default:
            break;
    }
